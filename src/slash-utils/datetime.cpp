  ...
  private:
    ...
    void print_help() {
      io::print(get_helpmsg({
        "Prints the locale datetime and interpret POSIX datetime flags",
        {
          "datetime",
          "datetime -f <text>" 
        },
        {
          {"-f", "--formatted", "Interpret POSIX datetime flags " + yellow + "(See datetime --posix)" + reset},
          {"-h", "--help", "Display this help message"},
          {"", "--posix", "Display all supported POSIX datetime flags"}
        },
        {
          {"datetime", "Prints the current locale's datetime"},
          {"datetime -f \"%Y-%m-%d %H:%M:%S %Z\"", "Prints the date, time, and timezone abbreviation"}
        },
        "",
        ""
      }));
