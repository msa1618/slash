    ...
    for(auto& arg : args) {
      ...
      if(arg == "--help") {
        io::print(get_helpmsg({
          "A helper command that prints ANSI color codes, aswell as manipulation codes",
          {
            "ansi",
            "ansi [option]"
          },
          {
            {"-f", "--foreground", "Prints foreground color codes instead of the default background"},
            {"-n", "--non-colo[u]r", "Prints ANSI manipulation codes"},
            {"-h", "--help", "Show this help message"},
          },
          {
            {"ansi", "Prints the background and foreground standard codes, aswell as the background 256 color palette"},
            {"ansi -f", "Prints the standard and 256 color codes in the foreground"}
          },
          "",
          ""
        }));
